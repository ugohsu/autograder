const SAMPLE = {
  roster: [
    { seq: 1, name: "試験 太郎", student_id: "2024001234", _confidence: 1.0 },
    {
      seq: 2, name: "山田 花子", student_id: "2024005678",
      _comment: "学籍番号の6桁目が '6' か '8' か判別しづらい。'8'として読んだが要確認。",
      _confidence: 0.6,
    },
  ],
};
...
previewBtn.addEventListener("click", async () => {
  ...
  try {
    const res = await fetch(`/batch/${window.BATCH_ID}/roster_import/preview`, {
      method: "POST",
...
function fieldHtml(row, field, label) {
  const value = row.cleaned[field];
  return `
    <div class="field"><label>${label}</label>
      <input type="text" class="ri-preview-field" data-idx="${row.index}" data-field="${field}"
  ...
  return `
    <div class="card import-row">
      <div class="card-header">
        ...
        <strong>${seqLabel}</strong>
        ${badges.join(" ")}
      </div>
      ${row.comment ? `<p class="hint">${escapeHtml(row.comment)}</p>` : ""}
      <div class="field-row">
        ${fieldHtml(row, "seq", "通し番号")}
        ${fieldHtml(row, "name", "氏名")}
...
previewListEl.addEventListener("change", (e) => {
  ...
  if (e.target.classList.contains("ri-preview-field")) {
    const field = e.target.dataset.field;
    const raw = e.target.value;
    row.cleaned[field] = raw === "" ? null : (field === "seq" ? Number(raw) : raw);
    renderPreview();
  }
});
...
commitBtn.addEventListener("click", async () => {
  ...
  try {
    const res = await fetch(`/batch/${window.BATCH_ID}/roster_import/commit`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    alertBox.innerHTML = `<div class="alert alert-success">${body.applied}件の氏名・学籍番号を反映しました。<a href="/batch/${window.BATCH_ID}">答案一覧で確認する</a></div>`;
    stepPreview.style.display = "none";
    jsonInput.value = "";
    jsonFile.value = "";
    previewRows = [];
